...
function greyConverter() {
    ...
    pixels = pixels.map(row => {
        return row.map(pixel => {
            let media = parseInt( pixel.reduce((total, valorActual) => {return total + valorActual;}, 0) / 3)
...
function blackAndWhiteConverter() {
    ...
    pixels = pixels.map(row => {
        return row.map(pixel => {
            let media = parseInt( pixel.reduce((total, valorActual) => {return total + valorActual;}, 0) / 3) < 128 ? 0 : 255;
            return [ media, media, media];
        });
    });
    handler.savePixels(pixels, outputPath);
...
function scaleDown() {
    ...
    let pixels = handler.getPixels();
    const pares = pixels.filter((element, indice) => {
        return indice %2 === 0;
    });      
    nuevaImagen = pares.map((fila) => {
        return fila.filter((element, indice) => {
          return indice %2 !== 0;
        });
    });
...
function dimBrightness(dimFactor) {
    ...
    pixels = pixels.map(row => {
        return row.map(pixel => {
            return pixel.map((value) => { return value / dimFactor; })
        });
    });
    handler.savePixels(pixels, outputPath);
}
...
function invertColors() {
    ...
    pixels = pixels.map(row => {
        return row.map(pixel => {
            return pixel.map((value) => { return 255 - value; })
        });
    });
    handler.savePixels(pixels, outputPath);
}
