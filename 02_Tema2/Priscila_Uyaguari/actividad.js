...
function blackAndWhiteConverter() {
    ...
    for (let fila = 0; fila < pixels.length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            let media = (pixels[fila][columna][0] + pixels[fila][columna][1] + pixels[fila][columna][2]) / 3;
            pixels[fila][columna] = (media < 128) ? [0, 0, 0] : [255, 255, 255];
...
function scaleDown() {
    ...
    for (let fila = 1; fila < pixels.length; fila += 2) {
        let nuevoPixel = [];
        for (let columna = 1; columna < pixels[fila].length; columna += 2) {
            nuevoPixel.push(pixels[fila][columna]);
        }
        nuevaImagen.push(nuevoPixel);
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let fila = 0; fila < pixels.length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            pixels[fila][columna] = [pixels[fila][columna][0] / dimFactor, pixels[fila][columna][1] / dimFactor, pixels[fila][columna][2] / dimFactor];
        }
    }
    handler.savePixels(pixels, outputPath);
}
...
function invertColors() {
    ...
    for (let fila = 0; fila < pixels.length; fila++) {
        for (let columna = 0; columna < pixels[fila].length; columna++) {
            pixels[fila][columna] = [255 - pixels[fila][columna][0], 255 - pixels[fila][columna][1], 255 - pixels[fila][columna][2]];
        }
