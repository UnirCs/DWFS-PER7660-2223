...
function blackAndWhiteConverter() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            ...
            let color = media < 128 ? [0,0,0] : [255, 255, 255];
            pixels[i][j] = color;
        }
    }
...
function scaleDown() {
    ...
    for (let i = 0; i < pixels.length; i+=2) {
        ...
        for (let j = 0; j < pixels[i].length; j+=2) {
            pixel.push(pixels[i][j]);
        }
        nuevaImagen.push(pixel);
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            let pixel = [(pixels[i][j][0] /dimFactor), (pixels[i][j][1] /dimFactor), (pixels[i][j][2] / dimFactor)];
            pixels[i][j] = pixel;
        }
    }
...
function invertColors() {
    ...
    for (let i = 0; i < pixels.length; i++) {
        for (let j = 0; j < pixels[i].length; j++) {
            let pixel = [255 - pixels[i][j][0], 255 - pixels[i][j][1], 255 - pixels[i][j][2]];
            pixels[i][j] = pixel;
        }
    }
