...
function greyConverter() {
    ...
    arrayPixels = pixels.map(row => {
        return row.map(pixel => {
            let media = parseInt((pixel[0] + pixel[1] + pixel[2])/3)
...
function blackAndWhiteConverter() {
    ...
    arrayPixels = pixels.map(row => {
        return row.map(pixel => {
            let media = parseInt((pixel[0] + pixel[1] + pixel[2])/3) < 128 ? 0 : 255;
          return [ media, media, media];
        });
      });
...
function scaleDown() {
    ...
    //Aqui tu codigo
    const filaspares = pixels.filter((fila, indice) => {
        return indice % 2 === 0;
      });
      
      arrayPixels = filaspares.map((fila) => {
        return fila.filter((elemento, indice) => {
          return indice % 2 !== 0;
        });
      });
    //Aqui tu codigo
...
function dimBrightness(dimFactor) {
    ...
    arrayPixels = pixels.map(row => {
        return row.map(pixel => {
          return [ parseInt((pixel[0])/dimFactor), parseInt((pixel[1])/dimFactor), parseInt((pixel[2])/dimFactor)];
        });
      });
...
function invertColors() {
    ...
    arrayPixels = pixels.map(row => {
        return row.map(pixel => {
          return [ (255 - pixel[0]), (255 - pixel[0]), (255 - pixel[0])];
        });
      });
