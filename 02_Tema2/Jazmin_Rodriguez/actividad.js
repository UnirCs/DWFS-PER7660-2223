...
function redConverter() {
    ...
    //Aqui tu codigo
    //console.log(pixels);
    resPixels = pixels.map(item => {
        return item.map(pixel => {
          return [pixel[0], 0, 0];
        });
      });
...
function blackAndWhiteConverter() {
    ...
    resPixels = pixels.map(item => {
        return item.map(pixel => {
            ...
            var green = pixel[1];
            var blue = pixel[2];
            var media = ((red + green + blue) / 3)> 128 ? 255 : 0;
...
function scaleDown() {
    ...
    //Aqui tu codigo
    resPixels = pixels.filter((item, idx) => {
        return idx % 2 === 0;}).map(pixel => {
            return pixel.filter((item, idx) => {
                return idx % 2 !== 0;
              });
        });
    pixels = resPixels;
...
function dimBrightness(dimFactor) {
    ...
    resPixels = pixels.map(item => {
        return item.map(pixel => {
            var red = pixel[0]/dimFactor;
            var green = pixel[1]/dimFactor;
            var blue = pixel[2]/dimFactor;
