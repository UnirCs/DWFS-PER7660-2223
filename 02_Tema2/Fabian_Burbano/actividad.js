...
function greyConverter() {
    ...
    for (let x = 0; x < pixels.length; x++) {
        for (let y = 0; y < pixels[x].length; y++) {
            let promedio = (pixels[x][y][0] + pixels[x][y][1] + pixels[x][y][2]) / 3;
            pixels[x][y][0] = promedio;
            pixels[x][y][1] = promedio;
            pixels[x][y][2] = promedio;
        }
    }
...
function blackAndWhiteConverter() {
    ...
    for (let x = 0; x < pixels.length; x++) {
        for (let y = 0; y < pixels[x].length; y++) {
            let promedio = (pixels[x][y][0] + pixels[x][y][1] + pixels[x][y][2]) / 3;
            pixels[x][y][0] = promedio < 45 ? 0 : 255;
            pixels[x][y][1] = promedio < 45 ? 0 : 255;
            pixels[x][y][2] = promedio < 45 ? 0 : 255;
        }
    }
...
function scaleDown() {
    ...
    for (let x = pixels.length - 1; x >= 0; x--) {
        if (x % 2 !== 0) {
            ...
            for (let y = pixels[x].length - 1; y >= 0; y = y - 2) {
                pixels[x].splice(y, 1);
            }
        }
...
function dimBrightness(dimFactor) {
    ...
    for (let x = 0; x < pixels.length; x++) {
        for (let y = 0; y < pixels[x].length; y++) {
            pixels[x][y][0] = dimFactor === undefined || dimFactor === 0 ? 0: pixels[x][y][0] / dimFactor;
            pixels[x][y][1] = dimFactor === undefined || dimFactor === 0 ? 0: pixels[x][y][1] / dimFactor;
            pixels[x][y][2] = dimFactor === undefined || dimFactor === 0 ? 0: pixels[x][y][2] / dimFactor;
        }
    }
