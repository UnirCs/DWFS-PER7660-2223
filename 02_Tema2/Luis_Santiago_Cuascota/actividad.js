...
function greyConverter() {
    ...
    pixels.forEach((fila)=>{
        fila.forEach((pixel)=>{
            const media = (pixel[0]+pixel[1]+pixel[2])/pixel.length;
...
function scaleDown() {
    ...
    pixels.forEach((fila, fIndex)=>{
        if (fIndex%2 === 0){
            const nuevaFila=[];
            fila.forEach((pixel, pIndex)=>{
                if(pIndex%2 == 0)
                    nuevaFila.push(pixel);
            });
...
switch (optionN) {
    ...
    case 7: dimBrightness(4); break;
    case 8: invertColors(); break;
    case 9: merge(0.4, 0.4); break;
    default: ejemplo();
}
