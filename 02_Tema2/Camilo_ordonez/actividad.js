...
function redConverter() {
    ...
    //generamos recorrridos con sentencia for

    for (let f=0; f < pixels.length; f++ ){
        //recorremos el ancho y a su vez el largo
        let width = pixels[i];
        for (let c=0; c< width.length; c++){
    
            //ponemos ceros los canales correspondientes
            pixels[f][c][1]=0;
            pixels[f][c][2]=0;
        }
    }
...
function dimBrightness(dimFactor) {
    ...
    for (let f = 0; f < pixels.length; f++) {
       ...
       for (let c = 0; c < pixels[f].length; c++) {
           pixels[f][c]= pixels[f][c]/dimFactor;
       }
}
...
function invertColors() {
    ...
    for (let f = 0; f < pixels.length; f++) {
        //recorremos el ancho     
       for (let c = 0; c < pixels[f].length; c++) {
           Math.abs(pixels[f][c][0]-=255)
           Math.abs(pixels[f][c][1]-=255)   
           Math.abs(pixels[f][c][2]-=255)  
       }
}
