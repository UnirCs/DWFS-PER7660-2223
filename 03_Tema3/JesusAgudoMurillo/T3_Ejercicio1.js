...
const obtenerLetraDni = (numeroDni) => numeroDni > 0 && numeroDni < 99999999 ? numeroDni + letras[numeroDni % 23] : null;
...
const formatStr = (cadena) => cadena.replace(/\s+/g, "").replace(/[^aeiou]/g, (letra) => letra.toUpperCase());
...
const esPalindromo = (cadena) => cadena.replace(/\s+/g, "").toLowerCase() === cadena.replace(/\s+/g, "").toLowerCase().split("").reverse().join("");

/* console.log(esPalindromo("Dabales arroz a la zorra el abad"));
 */
