...
const obtenerLetraDniFlecha = (dni) => dni > 0 && dni < 99999999 ? dni + letras[dni % 23] : null;
...
console.log(obtenerLetraDniFlecha(50487965));
/**
 * Funcion 2
 * 
 ...
 */
const formatStrFlecha = (cadena) => cadena.replaceAll(" ", "").replace(/[^aeiou]/g, (letra) => letra.toUpperCase());
function formatStr(cadena){
    return cadena.replaceAll(" ", "").replace(/[^aeiou]/g, (letra) => letra.toUpperCase());
}
console.log(formatStr("Esto es un Ejemplo"));
console.log(formatStrFlecha("Esto es un Ejemplo"));
/**
 * Funcion 3
 * 
 * Palindromos
 ...
 */
const esPalindromoFlecha = (cadena) =>{
    let cadenaConvert = cadena.toLowerCase().replace(/[\W_]/g, '');
    let reverseCadena = cadenaConvert.split('').reverse().join('');
    return cadenaConvert === reverseCadena;
};
...
console.log(esPalindromoFlecha("Dabale arroz a la zorra el abadw"));