 ...
 */
const duplicador = arr => arr.filter(el => el % 2 == 0).map(el => el * 2);
 ...
 */
const media = arr => arr.reduce((prev, curr) => prev + curr) / arr.length;
 ...
 * [1, 4] (2+6+7+1 /4 = 4)
 */
const randomMap = N => new Map(
    [...new Set([...Array(N)].map(e => Math.floor(Math.random() * (N - 1) + 1)))]
    .map((e, i, arr) => [e, arr.filter((e, j) => j <= i).reduce((a, b) => a + b) / (i + 1)])
);
