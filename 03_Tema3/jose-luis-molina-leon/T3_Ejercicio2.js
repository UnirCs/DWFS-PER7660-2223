...
const media = arr => arr.reduce((acc, curr) => acc + curr) / arr.length;
...
const eliminarDuplicados = arr => arr.filter((elem) => arr.indexOf(elem) === arr.lastIndexOf(elem));
...
const nCharConsec = (char, n, str) => str.split('').filter((c, i) => c === char && str.slice(i, i + n) === char.repeat(n)).length > 0;
...
const generador = (n) => {
    ...
    const arrayFiltrado = Array.from(arrayAleatorio).filter((elemento) => elemento !== 0).filter((elem, index, arr) => arr.indexOf(elem) === index);
    let mapa = new Map();
