...
function obtenerLetraDni(dni) {
    return (dni > 0 && dni < 99999999) ? dni % 23 : null;
}
const dni = 50487965;
console.log(`${dni}${letras[obtenerLetraDni(dni)]}`);
/**
 * Funcion 2
 * 
 ...
 */
function formatStr(valor) {
    const vocales = ['a', 'e', 'i', 'o', 'u'];
    let cadena = valor.trim().split(' ');
    let unido = '';
    ...
    for (let i = 0; i < unido.length; i++) {
        if (!vocales.includes(unido[i])) {
            palabra += unido[i].toUpperCase();
        } else {
            palabra += unido[i];
        }
    }
...
function esPalindromo(valor) {
    let cadena = valor.trim().split(' ').join('').toLowerCase();
