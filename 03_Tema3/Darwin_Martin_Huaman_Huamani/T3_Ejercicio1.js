 ...
 */
function formatStr(texto) {
    let str = texto.replace(/ /g, "");
    str = str.replace(/[^aeiou]+/g, (v) => v.toUpperCase())
    return str;
}

let texto = "Esto es un Ejemplo";
console.log(formatStr(texto));
...
function esPalindromo(text){
    ...
    let valor = true;
    let str = text.replace(/ /g, "").toUpperCase();
...
console.log(esPalindromo("arroz a la zorra"));
