...
function duplicador(arreglo) {
    const pares = arreglo.filter(value => value % 2 == 0);
    const duplicados = [];
    pares.forEach(element => duplicados.push(element * 2));
    return duplicados;

}
console.log("Ej1: Duplicador");
console.log(duplicador([1, 2, 3, 4, 5, 6, 7, 8]));
...
console.log('Ej2: Media');
console.log(media([2, 3, 5, 6, 4]));
...
function eliminarDuplicados(numeros) {
    ...
    numeros.forEach(element => {
        const repetidos = numeros.filter(value => value === element);
        if (repetidos.length == 1) {

            unicos.push(repetidos[0]);

        }
    });
    console.log("Ej:3 Eliminar duplicados")
    return unicos;

}
console.log(eliminarDuplicados([1, 3, 4, 5, 6, 7, 8, 8, 7, 6, 4, 5, 6, 7, 6, 5, 4, 3, 2, 3, 4, 5, 6]));
...
function nCharConsec(caracter, numero, cadena) {
    ...
    array.forEach((v, i) => {
        ...
        if (aux >= 0) {
            if (v == caracter) {
               // console.log(v);
                coincidencias = coincidencias + v;
            } else if (coincidencias.length == numero) {
                return;
            }
            else {
                aux = -1;
    ...
    return coincidencias.length == numero;
}
console.log("Ej4: Consecutivos");
...
function generador(n) {
    ...
    const enteros = numeros.filter(x => x != 0);
    const unicos = [];
    enteros.forEach(element => {
        const valorEncontrado = unicos.find(x => x == element)
    ...
    console.log("No repetidos " + unicos);
    ...
    unicos.forEach((x, i) => {
        ...
        cadena += `${x} + `;
        if (i == 0) {
            mapa.set(x, `${suma} / ${x} = ${suma / x}`);
        }else{
            let texto = `(${cadena.substring(0, cadena.length - 2)} / ${cont} = ${suma / cont})`;
            mapa.set(x, texto);
            cont++
        }
    });
