...
let esPar = (esNumero) => esNumero % 2 === 0;
console.log(esPar(1))
/**
 * Funcion 3
 * 
...
let formatStr = (palabra) =>{
    palabra = palabra.replaceAll(' ','')
    let arregloPalabra = palabra.split("");
    //arregloPalabra = arregloPalabra.map(x => x ===' ' ? '':x);
    arregloPalabra = arregloPalabra.map(x => !vocales.includes(x.toLowerCase()) ? x.toUpperCase():x );
...
let esPalindromo = (palindromo) =>{
    palindromo = palindromo.replaceAll(' ','').toLowerCase();
    let auxPalindromo = palindromo.split('').reverse();
    let palindromaOri = palindromo.split('').join('');
    let palindromaRev = auxPalindromo.join('');
    return palindromaOri === palindromaRev;
}
